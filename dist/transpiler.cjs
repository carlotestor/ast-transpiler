...
`;
  ...
  goTypeOfInitializer(initializer, printedValue) {
    switch (_optionalChain([initializer, 'optionalAccess', _272 => _272.kind])) {
      ...
      case _typescript2.default.SyntaxKind.BinaryExpression: {
        ...
        if (op === _typescript2.default.SyntaxKind.EqualsEqualsToken || op === _typescript2.default.SyntaxKind.EqualsEqualsEqualsToken || op === _typescript2.default.SyntaxKind.ExclamationEqualsToken || op === _typescript2.default.SyntaxKind.ExclamationEqualsEqualsToken) {
          return "bool";
        }
        break;
      }
  ...
  goEnclosingFunction(node) {
    let current = _optionalChain([node, 'optionalAccess', _273 => _273.parent]);
    while (current) {
      switch (current.kind) {
        case _typescript2.default.SyntaxKind.MethodDeclaration:
        case _typescript2.default.SyntaxKind.FunctionDeclaration:
  ...
  goTypeNameIsShadowed(scope, goType) {
    ...
    const visit = (n) => {
      ...
      const isBinding = n.kind === _typescript2.default.SyntaxKind.Parameter || n.kind === _typescript2.default.SyntaxKind.VariableDeclaration;
      if (isBinding && _optionalChain([n, 'access', _274 => _274.name, 'optionalAccess', _275 => _275.kind]) === _typescript2.default.SyntaxKind.Identifier) {
  ...
  goLocalIsSafeToType(scope, declaration, varName, goType) {
    ...
    const visit = (n) => {
      ...
      if (n.kind === _typescript2.default.SyntaxKind.Identifier && n.escapedText === varName && n !== declaration.name) {
        const parent = n.parent;
        if (_optionalChain([parent, 'optionalAccess', _276 => _276.kind]) === _typescript2.default.SyntaxKind.PropertyAccessExpression && parent.expression === n && _optionalChain([parent, 'access', _277 => _277.name, 'optionalAccess', _278 => _278.escapedText]) === "push") {
          safe = false;
          return;
        }
        if (_optionalChain([parent, 'optionalAccess', _279 => _279.kind]) === _typescript2.default.SyntaxKind.VariableDeclaration && parent.name === n) {
          return;
        }
        if (_optionalChain([parent, 'optionalAccess', _280 => _280.kind]) === _typescript2.default.SyntaxKind.PostfixUnaryExpression || _optionalChain([parent, 'optionalAccess', _281 => _281.kind]) === _typescript2.default.SyntaxKind.PrefixUnaryExpression) {
          const op = parent.operator;
          if (op === _typescript2.default.SyntaxKind.PlusPlusToken || op === _typescript2.default.SyntaxKind.MinusMinusToken) {
            safe = false;
            return;
        ...
        if (_optionalChain([parent, 'optionalAccess', _282 => _282.kind]) === _typescript2.default.SyntaxKind.SpreadElement) {
          safe = false;
          return;
        }
        if (_optionalChain([parent, 'optionalAccess', _283 => _283.kind]) === _typescript2.default.SyntaxKind.ArrayLiteralExpression && _optionalChain([parent, 'access', _284 => _284.parent, 'optionalAccess', _285 => _285.kind]) === _typescript2.default.SyntaxKind.BinaryExpression && parent.parent.left === parent && parent.parent.operatorToken.kind === _typescript2.default.SyntaxKind.EqualsToken) {
          safe = false;
          return;
        }
        if (_optionalChain([parent, 'optionalAccess', _286 => _286.kind]) === _typescript2.default.SyntaxKind.BinaryExpression && parent.left === n) {
          const op = parent.operatorToken.kind;
          if (op === _typescript2.default.SyntaxKind.EqualsToken) {
  ...
  getGoLocalType(declaration, parsedValue) {
    ...
    const sourceName = _optionalChain([declaration, 'access', _287 => _287.name, 'optionalAccess', _288 => _288.escapedText]);
    if (sourceName === void 0) {
      return "any";
    }
  ...
  printVariableDeclarationList(node, identation) {
    const declaration = node.declarations[0];
    if (_optionalChain([declaration, 'optionalAccess', _289 => _289.name, 'access', _290 => _290.kind]) === _typescript2.default.SyntaxKind.ArrayBindingPattern) {
      const arrayBindingPattern = declaration.name;
      const arrayBindingPatternElements = arrayBindingPattern.elements;
      const parsedArrayBindingElements = arrayBindingPatternElements.map((e) => this.printNode(e.name, 0));
      const syntheticName = parsedArrayBindingElements.join("") + "Variable";
...
`;
    ...
    if (_optionalChain([declaration, 'optionalAccess', _291 => _291.initializer, 'optionalAccess', _292 => _292.kind]) === _typescript2.default.SyntaxKind.AwaitExpression) {
...
${this.getIden(identation)}PanicOnError(${parsedName})`;
    ...
    if (_optionalChain([node, 'optionalAccess', _293 => _293.parent, 'optionalAccess', _294 => _294.kind]) === _typescript2.default.SyntaxKind.FirstStatement) {
      if (isNew) {
...
${this.getIden(identation)}PanicOnError(${varName})`;
  ...
  printConstructorDeclaration(node, identation) {
    ...
    let superCallParams = "";
    let hasSuperCall = false;
    _optionalChain([node, 'access', _295 => _295.body, 'optionalAccess', _296 => _296.statements, 'access', _297 => _297.forEach, 'call', _298 => _298((statement) => {
      if (_typescript2.default.isExpressionStatement(statement)) {
        const expression = statement.expression;
        if (_typescript2.default.isCallExpression(expression)) {
          const expressionText = expression.expression.getText().trim();
  ...
  printThisElementAccesssIfNeeded(node, identation) {
    const isAsync = true;
    const elementAccess = node.expression;
    if (_optionalChain([elementAccess, 'optionalAccess', _299 => _299.kind]) === _typescript2.default.SyntaxKind.ElementAccessExpression) {
      if (_optionalChain([elementAccess, 'optionalAccess', _300 => _300.expression, 'optionalAccess', _301 => _301.kind]) === _typescript2.default.SyntaxKind.ThisKeyword) {
        let parsedArg = _optionalChain([node, 'access', _302 => _302.arguments, 'optionalAccess', _303 => _303.length]) > 0 ? this.printNode(node.arguments[0], identation).trimStart() : "";
        const propName = this.printNode(elementAccess.argumentExpression, 0);
        const wrapperOpen = isAsync ? this.UKNOWN_PROP_ASYNC_WRAPPER_OPEN : this.UKNOWN_PROP_WRAPPER_OPEN;
        const wrapperClose = isAsync ? this.UNKOWN_PROP_ASYNC_WRAPPER_CLOSE : this.UNKOWN_PROP_WRAPPER_CLOSE;
        parsedArg = parsedArg ? ", " + parsedArg : "";
  ...
  printDynamicCall(node, identation) {
    const elementAccess = node.expression;
    if (_optionalChain([elementAccess, 'optionalAccess', _304 => _304.kind]) === _typescript2.default.SyntaxKind.ElementAccessExpression) {
      const argumentDepth = this.goExprDepth + (_optionalChain([node, 'access', _305 => _305.arguments, 'optionalAccess', _306 => _306.length]) > 0 ? 1 : 0);
      const parsedArg = _optionalChain([node, 'access', _307 => _307.arguments, 'optionalAccess', _308 => _308.length]) > 0 ? node.arguments.map((n) => this.goWithExprDepth(argumentDepth, () => this.printNode(n, identation).trimStart())).join(", ") : "";
  ...
  printWrappedUnknownThisProperty(node, identation = 0) {
    const type = this.getChecker().getResolvedSignature(node);
    if (_optionalChain([type, 'optionalAccess', _309 => _309.declaration]) === void 0) {
      const argumentDepth = this.goExprDepth + (_optionalChain([node, 'access', _310 => _310.arguments, 'optionalAccess', _311 => _311.length]) > 0 ? 1 : 0);
      let parsedArguments = _optionalChain([node, 'access', _312 => _312.arguments, 'optionalAccess', _313 => _313.map, 'call', _314 => _314((a) => this.goWithExprDepth(argumentDepth, () => this.printNode(a, identation).trimStart())), 'access', _315 => _315.join, 'call', _316 => _316(", ")]);
      parsedArguments = parsedArguments ? parsedArguments : "";
      const propName = _optionalChain([node, 'access', _317 => _317.expression, 'optionalAccess', _318 => _318.name, 'access', _319 => _319.escapedText]);
  ...
  printOutOfOrderCallExpressionIfAny(node, identation) {
    if (node.expression.kind === _typescript2.default.SyntaxKind.PropertyAccessExpression) {
      ...
      const leftSide = _optionalChain([node, 'access', _320 => _320.expression, 'optionalAccess', _321 => _321.expression]);
      const leftSideText = leftSide ? this.printNode(leftSide, 0) : void 0;
      if (leftSideText === this.THIS_TOKEN || leftSide.getFullText().indexOf("(this as any)") > -1) {
...
`;
  ...
  goScalarFamilyOfType(type) {
    ...
    const alias = _optionalChain([type, 'access', _322 => _322.aliasSymbol, 'optionalAccess', _323 => _323.escapedName]);
    switch (alias) {
      case "Str":
      case "Int":
      case "Num":
  ...
  goDeclaredTypeOfIdentifier(node) {
    if (_optionalChain([node, 'optionalAccess', _324 => _324.kind]) !== _typescript2.default.SyntaxKind.Identifier) {
      return void 0;
    }
    let symbol;
    try {
    ...
    const decl = _optionalChain([symbol, 'optionalAccess', _325 => _325.valueDeclaration]);
    if (decl === void 0 || decl.kind !== _typescript2.default.SyntaxKind.VariableDeclaration) {
      return void 0;
    }
    if (decl.initializer === void 0 || _optionalChain([decl, 'access', _326 => _326.name, 'optionalAccess', _327 => _327.kind]) !== _typescript2.default.SyntaxKind.Identifier) {
      return void 0;
    }
  ...
  goPointerTypeOfExpression(node, printedText) {
    ...
    if (_optionalChain([node, 'optionalAccess', _328 => _328.kind]) === _typescript2.default.SyntaxKind.CallExpression) {
  ...
  printInlineTruthy(node) {
    if (_optionalChain([node, 'optionalAccess', _329 => _329.kind]) !== _typescript2.default.SyntaxKind.Identifier) {
      return void 0;
    }
    const goType = this.goDeclaredTypeOfIdentifier(node);
    if (goType === void 0) {
  ...
  goIsControlClauseCondition(node) {
    const parent = _optionalChain([node, 'optionalAccess', _330 => _330.parent]);
    switch (_optionalChain([parent, 'optionalAccess', _331 => _331.kind])) {
      case _typescript2.default.SyntaxKind.IfStatement:
      case _typescript2.default.SyntaxKind.WhileStatement:
  ...
  printCondition(node, identation) {
    if (_optionalChain([node, 'optionalAccess', _332 => _332.kind]) === _typescript2.default.SyntaxKind.Identifier) {
  ...
  goDerefComparableWith(ptrNode, ptrText, otherNode) {
    const pointee = _optionalChain([this, 'access', _333 => _333.goPointerTypeOfExpression, 'call', _334 => _334(ptrNode, ptrText), 'optionalAccess', _335 => _335.substring, 'call', _336 => _336(1)]);
    if (pointee === void 0) {
      return false;
    }
    switch (_optionalChain([otherNode, 'optionalAccess', _337 => _337.kind])) {
      case _typescript2.default.SyntaxKind.StringLiteral:
      case _typescript2.default.SyntaxKind.NoSubstitutionTemplateLiteral:
  ...
  printInlineEquality(left, right, leftText, rightText, isEq) {
    ...
    const lRepeatable = _optionalChain([left, 'optionalAccess', _338 => _338.kind]) === _typescript2.default.SyntaxKind.Identifier;
    const rRepeatable = _optionalChain([right, 'optionalAccess', _339 => _339.kind]) === _typescript2.default.SyntaxKind.Identifier;
  ...
  printParenthesizedExpression(node, identation) {
    const expression = node.expression;
    if (_optionalChain([expression, 'optionalAccess', _340 => _340.kind]) === _typescript2.default.SyntaxKind.AsExpression) {
      return this.getIden(identation) + this.printNode(expression, 0);
    }
    if (_optionalChain([expression, 'optionalAccess', _341 => _341.kind]) === _typescript2.default.SyntaxKind.ArrowFunction) {
      return "";
    }
  ...
  printExpressionStatement(node, identation) {
    if (_optionalChain([node, 'optionalAccess', _342 => _342.expression, 'optionalAccess', _343 => _343.kind]) === _typescript2.default.SyntaxKind.AsExpression) {
...
${this.getIden(identation)}PanicOnError(${returnRandName})`;
  ...
  printReturnStatement(node, identation) {
    ...
    if (_optionalChain([node, 'optionalAccess', _344 => _344.expression, 'optionalAccess', _345 => _345.kind]) === _typescript2.default.SyntaxKind.AsExpression) {
      node = node.expression;
    }
    if (_optionalChain([node, 'optionalAccess', _346 => _346.expression, 'optionalAccess', _347 => _347.kind]) === _typescript2.default.SyntaxKind.AwaitExpression) {
...
${this.getIden(identation)}${returnStatement}`;
  ...
  printArrayPushCall(node, identation, name = void 0, parsedArg = void 0) {
    ...
    if (_optionalChain([name, 'optionalAccess', _348 => _348.startsWith, 'call', _349 => _349("GetValue")])) {
...
${this.getIden(identation)}`;
  ...
  printThrowStatement(node, identation) {
    ...
    if (node.expression.kind === _typescript2.default.SyntaxKind.NewExpression) {
      const expression = node.expression;
      const argumentsExp = _nullishCoalesce(_optionalChain([expression, 'optionalAccess', _350 => _350.arguments]), () => ( []));
      const parsedArg = _nullishCoalesce(argumentsExp.map((n) => this.printNode(n, 0)).join(","), () => ( ""));
      const newExpression = this.printNode(expression.expression, 0);
      if (expression.expression.kind === _typescript2.default.SyntaxKind.Identifier) {
        const id = expression.expression;
        const symbol = this.getChecker().getSymbolAtLocation(expression.expression);
        if (symbol) {
          const declarations = _nullishCoalesce(_optionalChain([this, 'access', _351 => _351.getChecker, 'call', _352 => _352(), 'access', _353 => _353.getDeclaredTypeOfSymbol, 'call', _354 => _354(symbol), 'access', _355 => _355.symbol, 'optionalAccess', _356 => _356.declarations]), () => ( []));
          const isClassDeclaration = declarations.find((l) => l.kind === _typescript2.default.SyntaxKind.InterfaceDeclaration || l.kind === _typescript2.default.SyntaxKind.ClassDeclaration);
          if (isClassDeclaration) {
          } else {
...
${this.getIden(identation)}return nil`;
  ...
  printBinaryExpression(node, identation) {
    ...
    if (operatorToken.kind === _typescript2.default.SyntaxKind.EqualsToken) {
      ...
      if (_optionalChain([right, 'optionalAccess', _357 => _357.kind]) === _typescript2.default.SyntaxKind.AwaitExpression || rightSide.startsWith("<-this.callInternal")) {
        const leftParsed = this.printNode(left, 0);
        const awaited = _optionalChain([right, 'optionalAccess', _358 => _358.kind]) === _typescript2.default.SyntaxKind.AwaitExpression ? this.printNode(right, identation) : rightSide;
        return `
...
${tryBodyBlock}
  ...
  printNewExpression(node, identation) {
    let expression = _optionalChain([node, 'access', _359 => _359.expression, 'optionalAccess', _360 => _360.escapedText]);
    expression = expression ? expression : this.printNode(node.expression);
...
var JavaTranspiler = class extends BaseTranspiler {
  ...
  countRequiredParameters(declaration) {
    const params = _nullishCoalesce(_optionalChain([declaration, 'optionalAccess', _361 => _361.parameters]), () => ( []));
    let required = 0;
  ...
  printArgsForCallExpression(node, identation) {
    ...
    const isThisCall = _optionalChain([callee, 'optionalAccess', _362 => _362.kind]) === _typescript2.default.SyntaxKind.PropertyAccessExpression && _optionalChain([callee, 'access', _363 => _363.expression, 'optionalAccess', _364 => _364.kind]) === _typescript2.default.SyntaxKind.ThisKeyword;
    if (isThisCall && args.length > 0) {
      ...
      if (isNullish) {
        ...
        const declaration = _optionalChain([signature, 'optionalAccess', _365 => _365.declaration]);
        if (declaration !== void 0) {
  ...
  printIdentifier(node) {
    ...
    const isInsideNewExpression = _optionalChain([node, 'optionalAccess', _366 => _366.parent, 'optionalAccess', _367 => _367.kind]) === _typescript2.default.SyntaxKind.NewExpression;
    const isInsideCatch = _optionalChain([node, 'optionalAccess', _368 => _368.parent, 'optionalAccess', _369 => _369.kind]) === _typescript2.default.SyntaxKind.ThrowStatement;
    const isLeftSide = _optionalChain([node, 'optionalAccess', _370 => _370.parent, 'optionalAccess', _371 => _371.name]) === node || _optionalChain([node, 'optionalAccess', _372 => _372.parent, 'optionalAccess', _373 => _373.left]) === node;
    const isCallOrPropertyAccess = _optionalChain([node, 'optionalAccess', _374 => _374.parent, 'optionalAccess', _375 => _375.kind]) === _typescript2.default.SyntaxKind.PropertyAccessExpression || _optionalChain([node, 'optionalAccess', _376 => _376.parent, 'optionalAccess', _377 => _377.kind]) === _typescript2.default.SyntaxKind.ElementAccessExpression;
    if (!isLeftSide && !isCallOrPropertyAccess && !isInsideCatch && !isInsideNewExpression) {
      const type = this.getChecker().getTypeAtLocation(node);
      const typeSymbol = _optionalChain([type, 'optionalAccess', _378 => _378.symbol]);
      if (typeSymbol !== void 0) {
        const decl = _nullishCoalesce(_optionalChain([typeSymbol, 'optionalAccess', _379 => _379.declarations]), () => ( []));
        let isBuiltIn = void 0;
        if (decl.length > 0) {
          isBuiltIn = decl[0].getSourceFile().fileName.indexOf("typescript") > -1;
        }
        if (isBuiltIn !== void 0 && !isBuiltIn) {
          ...
          if (symbol) {
            ...
            if (first.kind === _typescript2.default.SyntaxKind.ImportSpecifier) {
              ...
              if (_optionalChain([importedSymbol, 'optionalAccess', _380 => _380.declarations, 'access', _381 => _381[0], 'optionalAccess', _382 => _382.kind]) === _typescript2.default.SyntaxKind.ClassDeclaration) {
                isClassDeclaration = true;
              }
            }
          }
  ...
  printConstructorDeclaration(node, identation) {
    ...
    let superCallParams = "";
    let hasSuperCall = false;
    _optionalChain([node, 'access', _383 => _383.body, 'optionalAccess', _384 => _384.statements, 'access', _385 => _385.forEach, 'call', _386 => _386((statement) => {
      if (_typescript2.default.isExpressionStatement(statement)) {
        const expression = statement.expression;
        if (_typescript2.default.isCallExpression(expression)) {
          const expressionText = expression.expression.getText().trim();
  ...
  printDynamicCall(node, identation) {
    const elementAccess = node.expression;
    if (_optionalChain([elementAccess, 'optionalAccess', _387 => _387.kind]) === _typescript2.default.SyntaxKind.ElementAccessExpression) {
      const parsedArg = _optionalChain([node, 'access', _388 => _388.arguments, 'optionalAccess', _389 => _389.length]) > 0 ? node.arguments.map((n) => this.printNode(n, identation).trimStart()).join(", ") : "";
      const target = this.printNode(elementAccess.expression, 0);
      const propName = this.printNode(elementAccess.argumentExpression, 0);
      const argsArray = `new Object[] { ${parsedArg} }`;
      const open = this.DYNAMIC_CALL_OPEN;
  ...
  getExpressionStatementPrefixesIfAny(node, identation) {
    const finalVars = [];
    if (_optionalChain([node, 'access', _390 => _390.expression, 'optionalAccess', _391 => _391.kind]) === _typescript2.default.SyntaxKind.CallExpression) {
  ...
  printWrappedUnknownThisProperty(node) {
    const type = this.getChecker().getResolvedSignature(node);
    if (_optionalChain([type, 'optionalAccess', _392 => _392.declaration]) === void 0) {
      let parsedArguments = _optionalChain([node, 'access', _393 => _393.arguments, 'optionalAccess', _394 => _394.map, 'call', _395 => _395((a) => this.printNode(a, 0)), 'access', _396 => _396.join, 'call', _397 => _397(", ")]);
      parsedArguments = parsedArguments ? parsedArguments : "";
      const propName = _optionalChain([node, 'access', _398 => _398.expression, 'optionalAccess', _399 => _399.name, 'access', _400 => _400.escapedText]);
      const isAsyncDecl = _optionalChain([node, 'optionalAccess', _401 => _401.parent, 'optionalAccess', _402 => _402.kind]) === _typescript2.default.SyntaxKind.AwaitExpression;
      const argsArray = `new Object[] { ${parsedArguments} }`;
      const open = this.DYNAMIC_CALL_OPEN;
  ...
  printOutOfOrderCallExpressionIfAny(node, identation) {
    if (node.expression.kind === _typescript2.default.SyntaxKind.PropertyAccessExpression) {
      ...
      const leftSide = _optionalChain([node, 'access', _403 => _403.expression, 'optionalAccess', _404 => _404.expression]);
      const leftSideText = leftSide ? this.printNode(leftSide, 0) : void 0;
      if (leftSideText === this.THIS_TOKEN || leftSide.getFullText().indexOf("(this as any)") > -1) {
        const res = this.printWrappedUnknownThisProperty(node);
        if (res)
  ...
  getVarMethodIfAny(node) {
    let current = _optionalChain([node, 'optionalAccess', _405 => _405.parent]);
    while (current) {
      if (_typescript2.default.isMethodDeclaration(current) || _typescript2.default.isFunctionDeclaration(current)) {
        return String(_nullishCoalesce(_optionalChain([current, 'access', _406 => _406.name, 'optionalAccess', _407 => _407.escapedText]), () => ( "")));
      }
      current = current.parent;
    }
  ...
  getVarClassIfAny(node) {
    let current = _optionalChain([node, 'optionalAccess', _408 => _408.parent]);
    while (current) {
      if (_typescript2.default.isClassDeclaration(current)) {
        return String(_nullishCoalesce(_optionalChain([current, 'access', _409 => _409.name, 'optionalAccess', _410 => _410.escapedText]), () => ( "")));
      }
      current = current.parent;
    }
    return "";
  ...
  getVarKey(node) {
    const varName = _nullishCoalesce(_optionalChain([node, 'optionalAccess', _411 => _411.escapedText]), () => ( _optionalChain([node, 'optionalAccess', _412 => _412.name, 'optionalAccess', _413 => _413.escapedText])));
    if (!varName) {
      return "";
    }
  ...
  isNullishLiteral(node) {
    return _optionalChain([node, 'optionalAccess', _414 => _414.kind]) === _typescript2.default.SyntaxKind.NullKeyword || _optionalChain([node, 'optionalAccess', _415 => _415.kind]) === _typescript2.default.SyntaxKind.Identifier && node.escapedText === "undefined";
  }
  ...
  printNativeEqualityIfProvable(node, leftText, rightText) {
    ...
    const checker = this.getChecker();
    const leftFamily = this.equalityOperandFamily(_optionalChain([checker, 'optionalAccess', _416 => _416.getTypeAtLocation, 'call', _417 => _417(node.left)]));
    const rightFamily = this.equalityOperandFamily(_optionalChain([checker, 'optionalAccess', _418 => _418.getTypeAtLocation, 'call', _419 => _419(node.right)]));
  ...
  isVarargsArrayReference(node, depth = 0) {
    ...
    const symbol = this.getChecker().getSymbolAtLocation(node);
    const declaration = _nullishCoalesce(_optionalChain([symbol, 'optionalAccess', _420 => _420.valueDeclaration]), () => ( _optionalChain([symbol, 'optionalAccess', _421 => _421.declarations, 'optionalAccess', _422 => _422[0]])));
    if (!declaration) {
      return false;
    }
  ...
  isJavaListType(type) {
    ...
    return _optionalChain([type, 'access', _423 => _423.target, 'optionalAccess', _424 => _424.symbol, 'optionalAccess', _425 => _425.escapedName]) === "ReadonlyArray";
  }
  ...
  isJavaPrimitiveForCounter(node) {
    ...
    if (!incrementor || _optionalChain([incrementor, 'access', _426 => _426.operand, 'optionalAccess', _427 => _427.kind]) !== _typescript2.default.SyntaxKind.Identifier || incrementor.operand.escapedText !== node.escapedText) {
      return false;
    }
  ...
  isJavaMapType(type) {
    ...
    const declarations = _nullishCoalesce(_optionalChain([type, 'access', _428 => _428.getSymbol, 'call', _429 => _429(), 'optionalAccess', _430 => _430.declarations]), () => ( []));
...
`;
  ...
  isJavaMapStructureType(type) {
    ...
    const declaration = _optionalChain([symbol, 'optionalAccess', _431 => _431.declarations, 'optionalAccess', _432 => _432[0]]);
    const fileName = _optionalChain([declaration, 'optionalAccess', _433 => _433.getSourceFile, 'optionalCall', _434 => _434(), 'optionalAccess', _435 => _435.fileName]);
  ...
  tupleRequiredElementCount(type) {
    const flags = _nullishCoalesce(_nullishCoalesce(_optionalChain([type, 'optionalAccess', _436 => _436.target, 'optionalAccess', _437 => _437.elementFlags]), () => ( _optionalChain([type, 'optionalAccess', _438 => _438.elementFlags]))), () => ( []));
    let required = 0;
  ...
  isLeftSideOfAssignment(node) {
    const parent = node.parent;
    if (_optionalChain([parent, 'optionalAccess', _439 => _439.kind]) !== _typescript2.default.SyntaxKind.BinaryExpression || parent.left !== node) {
      return false;
    }
  ...
  javaNativeConcat(node) {
    if (_optionalChain([node, 'optionalAccess', _440 => _440.operatorToken, 'optionalAccess', _441 => _441.kind]) !== _typescript2.default.SyntaxKind.PlusToken) {
      return false;
    }
  ...
  javaNativeArithmeticKind(node) {
    const op = _optionalChain([node, 'optionalAccess', _442 => _442.operatorToken, 'optionalAccess', _443 => _443.kind]);
  ...
  getObjectLiteralFromCallExpressionArguments(node) {
    const res = [];
    if (!_optionalChain([node, 'optionalAccess', _444 => _444.arguments])) {
      return res;
    }
    const args = node.arguments;
  ...
  getBinaryExpressionPrefixes(node, identation) {
    let right = _optionalChain([node, 'optionalAccess', _445 => _445.right]);
    if (_optionalChain([right, 'optionalAccess', _446 => _446.kind]) === _typescript2.default.SyntaxKind.AwaitExpression) {
  ...
  analyzeFinalVars(fnBody) {
    ...
    const symbolIdOf = (n) => {
      try {
        const checker = this.getChecker();
        const sym = _optionalChain([checker, 'optionalAccess', _447 => _447.getSymbolAtLocation, 'optionalCall', _448 => _448(n)]);
        const decl = _nullishCoalesce(_optionalChain([sym, 'optionalAccess', _449 => _449.declarations, 'optionalAccess', _450 => _450[0]]), () => ( _optionalChain([sym, 'optionalAccess', _451 => _451.valueDeclaration])));
        if (decl)
    ...
    const discoverPotentialReassignments = (node) => {
      if (!node)
        return;
      if (node.kind === _typescript2.default.SyntaxKind.BinaryExpression) {
        if (_optionalChain([node, 'access', _452 => _452.left, 'optionalAccess', _453 => _453.kind]) === _typescript2.default.SyntaxKind.Identifier) {
          reassignedSyms.add(symbolIdOf(node.left));
        } else if (node.operatorToken.kind === _typescript2.default.SyntaxKind.EqualsToken && _optionalChain([node, 'access', _454 => _454.left, 'optionalAccess', _455 => _455.kind]) === _typescript2.default.SyntaxKind.ArrayLiteralExpression) {
          for (const elem of _nullishCoalesce(node.left.elements, () => ( []))) {
            if (_optionalChain([elem, 'optionalAccess', _456 => _456.kind]) === _typescript2.default.SyntaxKind.Identifier) {
              reassignedSyms.add(symbolIdOf(elem));
            }
          }
        }
      }
      if ((node.kind === _typescript2.default.SyntaxKind.PrefixUnaryExpression || node.kind === _typescript2.default.SyntaxKind.PostfixUnaryExpression) && this.isIncDecOperator(node.operator) && _optionalChain([node, 'access', _457 => _457.operand, 'optionalAccess', _458 => _458.kind]) === _typescript2.default.SyntaxKind.Identifier) {
        reassignedSyms.add(symbolIdOf(node.operand));
      }
      if (node.kind === _typescript2.default.SyntaxKind.Identifier) {
        const name = node.escapedText;
        if (name && name !== "undefined" && !_optionalChain([name, 'access', _459 => _459.startsWith, 'optionalCall', _460 => _460("null")])) {
    ...
    const visitExprInObjLit = (n) => {
      ...
      if (n.kind === _typescript2.default.SyntaxKind.Identifier) {
        ...
        if (name && name !== "undefined" && !_optionalChain([name, 'access', _461 => _461.startsWith, 'optionalCall', _462 => _462("null")])) {
      ...
      if (n.kind === _typescript2.default.SyntaxKind.CallExpression) {
        _optionalChain([n, 'access', _463 => _463.arguments, 'optionalAccess', _464 => _464.forEach, 'call', _465 => _465(visitExprInObjLit)]);
        if (_optionalChain([n, 'access', _466 => _466.expression, 'optionalAccess', _467 => _467.kind]) === _typescript2.default.SyntaxKind.PropertyAccessExpression) {
      ...
      if (n.kind === _typescript2.default.SyntaxKind.ElementAccessExpression) {
        ...
        while (_optionalChain([left, 'optionalAccess', _468 => _468.kind]) === _typescript2.default.SyntaxKind.ElementAccessExpression) {
      ...
      if (n.kind === _typescript2.default.SyntaxKind.ObjectLiteralExpression) {
        _optionalChain([n, 'access', _469 => _469.properties, 'optionalAccess', _470 => _470.forEach, 'call', _471 => _471((p) => {
          if (p.initializer)
    ...
    const walk = (node) => {
      ...
      if ((node.kind === _typescript2.default.SyntaxKind.PrefixUnaryExpression || node.kind === _typescript2.default.SyntaxKind.PostfixUnaryExpression) && this.isIncDecOperator(node.operator) && _optionalChain([node, 'access', _474 => _474.operand, 'optionalAccess', _475 => _475.kind]) === _typescript2.default.SyntaxKind.Identifier) {
      ...
      if (node.kind === _typescript2.default.SyntaxKind.ObjectLiteralExpression) {
        _optionalChain([node, 'access', _476 => _476.properties, 'optionalAccess', _477 => _477.forEach, 'call', _478 => _478((prop) => {
          if (prop.initializer) {
      ...
      if (node.kind === _typescript2.default.SyntaxKind.TryStatement) {
        ...
        walkBlockAndBump(_optionalChain([node, 'access', _479 => _479.catchClause, 'optionalAccess', _480 => _480.block]));
  ...
  getObjectLiteralId(node) {
    ...
    const fileName = _nullishCoalesce(_optionalChain([node, 'access', _481 => _481.getSourceFile, 'optionalCall', _482 => _482(), 'optionalAccess', _483 => _483.fileName]), () => ( ""));
  ...
  printVariableDeclarationList(node, identation) {
    ...
    if (this.removeVariableDeclarationForFunctionExpression && _optionalChain([declaration, 'optionalAccess', _484 => _484.initializer]) && _typescript2.default.isFunctionExpression(declaration.initializer)) {
      return this.printNode(declaration.initializer, identation).trimEnd();
    }
    if (_optionalChain([declaration, 'optionalAccess', _485 => _485.name, 'access', _486 => _486.kind]) === _typescript2.default.SyntaxKind.ArrayBindingPattern) {
      const arrayBindingPattern = declaration.name;
      const arrayBindingPatternElements = arrayBindingPattern.elements;
...
`;
    ...
    const isNew = _optionalChain([declaration, 'optionalAccess', _487 => _487.initializer]) && declaration.initializer.kind === _typescript2.default.SyntaxKind.NewExpression;
    const varToken = isNew ? "var " : this.VAR_TOKEN + " ";
    if (!declaration.initializer) {
      return this.getIden(identation) + "Object " + this.printNode(declaration.name) + " = " + this.UNDEFINED_TOKEN;
    }
  ...
  printThisKeyword(node, identation) {
    let current = _optionalChain([node, 'optionalAccess', _488 => _488.parent]);
    while (current) {
      ...
      current = _optionalChain([current, 'optionalAccess', _489 => _489.parent]);
    }
    return this.THIS_TOKEN;
  }
  transformPropertyAcessExpressionIfNeeded(node) {
  ...
  printCustomDefaultValueIfNeeded(node) {
    ...
    if (_optionalChain([node, 'optionalAccess', _490 => _490.escapedText]) === "undefined" && _optionalChain([this, 'access', _491 => _491.getChecker, 'call', _492 => _492(), 'access', _493 => _493.getTypeAtLocation, 'call', _494 => _494(_optionalChain([node, 'optionalAccess', _495 => _495.parent])), 'optionalAccess', _496 => _496.flags]) === _typescript2.default.TypeFlags.Number) {
      return this.UNDEFINED_TOKEN;
    }
    return void 0;
  }
  ...
  isPureInitializer(node) {
    switch (_optionalChain([node, 'optionalAccess', _497 => _497.kind])) {
      case _typescript2.default.SyntaxKind.NullKeyword:
      case _typescript2.default.SyntaxKind.TrueKeyword:
      case _typescript2.default.SyntaxKind.FalseKeyword:
      case _typescript2.default.SyntaxKind.StringLiteral:
...
`;
  ...
  printFinalOutsideMethodVariableWrappersIfAny(node, identation) {
    const parameters = _optionalChain([node, 'optionalAccess', _498 => _498.parameters]);
  ...
  printInsideMethodVariableWrappersIfAny(node, identation) {
    const parameters = _optionalChain([node, 'optionalAccess', _499 => _499.parameters]);
    const finalVarWrappers = [];
    if (parameters) {
      const isAsyncMethod = this.isAsyncFunction(node);
      parameters.forEach((param) => {
  ...
  printReplaceAllCall(node, identation, name = void 0, parsedArg = void 0, parsedArg2 = void 0) {
    const pattern = this.stringLiteralArgument(_optionalChain([node, 'optionalAccess', _500 => _500.arguments, 'optionalAccess', _501 => _501[0]]));
    const replacement = this.stringLiteralArgument(_optionalChain([node, 'optionalAccess', _502 => _502.arguments, 'optionalAccess', _503 => _503[1]]));
    const receiver = this.sideEffectFreeReceiver(_optionalChain([node, 'optionalAccess', _504 => _504.expression]));
  ...
  printThrowStatement(node, identation) {
    ...
    if (node.expression.kind === _typescript2.default.SyntaxKind.NewExpression) {
      const expression = node.expression;
      const argumentsExp = _nullishCoalesce(_optionalChain([expression, 'optionalAccess', _505 => _505.arguments]), () => ( []));
      const parsedArg = _nullishCoalesce(argumentsExp.map((n) => this.printNode(n, 0)).join(","), () => ( ""));
      const newExpression = this.printNode(expression.expression, 0);
      if (expression.expression.kind === _typescript2.default.SyntaxKind.Identifier) {
        const id = expression.expression;
        ...
        const symbol = this.getChecker().getSymbolAtLocation(expression.expression);
        if (symbol) {
          const declarations = _nullishCoalesce(_optionalChain([this, 'access', _506 => _506.getChecker, 'call', _507 => _507(), 'access', _508 => _508.getDeclaredTypeOfSymbol, 'call', _509 => _509(symbol), 'access', _510 => _510.symbol, 'optionalAccess', _511 => _511.declarations]), () => ( []));
...
var _RustTranspiler = class _RustTranspiler extends BaseTranspiler {
  ...
  printVariableDeclarationList(node, identation) {
    ...
    if (_optionalChain([declaration, 'optionalAccess', _512 => _512.name, 'access', _513 => _513.kind]) === SyntaxKind4.ArrayBindingPattern) {
...
${classMethods}
  ...
  printFunctionDefinition(node, identation) {
    const name = _nullishCoalesce(_optionalChain([node, 'access', _514 => _514.name, 'optionalAccess', _515 => _515.escapedText]), () => ( ""));
    const params = node.parameters;
  ...
  printNewExpression(node, identation) {
    let expression = _optionalChain([node, 'access', _516 => _516.expression, 'optionalAccess', _517 => _517.escapedText]);
    expression = expression ? expression : this.printNode(node.expression);
...
${idn}}`;
  ...
  printIfStatement(node, identation) {
    ...
    const elseStatement = node.elseStatement;
    if (_optionalChain([elseStatement, 'optionalAccess', _518 => _518.kind]) === SyntaxKind4.Block) {
      ifComplete += ` else${this.printBlock(elseStatement, identation)}`;
    } else if (_optionalChain([elseStatement, 'optionalAccess', _519 => _519.kind]) === SyntaxKind4.IfStatement) {
...
${this.getIden(identation)}})`;
  ...
  printIncludesCall(node, identation, name = void 0, parsedArg = void 0) {
    const pRef = _optionalChain([parsedArg, 'optionalAccess', _520 => _520.startsWith, 'call', _521 => _521("Value::")]) ? `&${parsedArg}` : `&${parsedArg}`;
  ...
  printTryStatement(node, identation) {
    ...
    const catchBody = node.catchClause.block.statements.map((s) => this.printNode(s, identation + 1)).join("\n");
    const rawName = _optionalChain([node, 'access', _522 => _522.catchClause, 'optionalAccess', _523 => _523.variableDeclaration, 'optionalAccess', _524 => _524.name, 'optionalAccess', _525 => _525.escapedText]);
...
var CppTranspiler = class extends BaseTranspiler {
  ...
  printConstructorDeclaration(node, identation) {
    ...
    let superCallParams = "";
    let hasSuperCall = false;
    _optionalChain([node, 'access', _526 => _526.body, 'optionalAccess', _527 => _527.statements, 'access', _528 => _528.forEach, 'call', _529 => _529((statement) => {
      if (_typescript2.default.isExpressionStatement(statement)) {
        const expression = statement.expression;
        if (_typescript2.default.isCallExpression(expression)) {
          const expressionText = expression.expression.getText().trim();
...
`;
  ...
  printVariableDeclarationList(node, identation) {
    const declaration = node.declarations[0];
    if (_optionalChain([declaration, 'optionalAccess', _530 => _530.name, 'access', _531 => _531.kind]) === _typescript2.default.SyntaxKind.ArrayBindingPattern) {
      const arrayBindingPattern = declaration.name;
      const arrayBindingPatternElements = arrayBindingPattern.elements;
      const parsedArrayBindingElements = arrayBindingPatternElements.map((e) => this.printNode(e.name, 0));
      const syntheticName = parsedArrayBindingElements.join("") + "Variable";
...
`;
  ...
  printThrowStatement(node, identation) {
    ...
    if (node.expression.kind === _typescript2.default.SyntaxKind.NewExpression) {
      const expression = node.expression;
      const argumentsExp = _nullishCoalesce(_optionalChain([expression, 'optionalAccess', _532 => _532.arguments]), () => ( []));
      const parsedArg = _nullishCoalesce(argumentsExp.map((n) => this.printNode(n, 0)).join(", "), () => ( ""));
      const newExpression = this.printNode(expression.expression, 0);
      if (expression.expression.kind === _typescript2.default.SyntaxKind.Identifier) {
        const id = expression.expression;
        const symbol = this.getChecker().getSymbolAtLocation(expression.expression);
        if (symbol) {
          const declarations = _nullishCoalesce(_optionalChain([this, 'access', _533 => _533.getChecker, 'call', _534 => _534(), 'access', _535 => _535.getDeclaredTypeOfSymbol, 'call', _536 => _536(symbol), 'access', _537 => _537.symbol, 'optionalAccess', _538 => _538.declarations]), () => ( []));
          const isClassDeclaration = declarations.find((l) => l.kind === _typescript2.default.SyntaxKind.InterfaceDeclaration || l.kind === _typescript2.default.SyntaxKind.ClassDeclaration);
          if (isClassDeclaration) {
...
function getProgramAndTypeCheckerFromMemory(rootDir, text, options = {}, cache) {
  ...
  const program = _typescript2.default.createProgram({
    ...
    host,
    oldProgram: _optionalChain([cache, 'optionalAccess', _539 => _539.memoryOldProgram])
  });
  if (cache !== void 0) {
